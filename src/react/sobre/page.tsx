...
export default function SobrePage({ content }: { content?: SiteContent }) {
  ...
  return <main>
    ...
    <section className="section">
      <div className="section-heading split"><div><span className="eyebrow">O propósito</span><h2>{about.projectTitle}</h2></div><div className="about-copy">{about.projectParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div>
    ...
    <section className="about-stories">
      <article><span className="eyebrow">O território</span><h2>{about.territoryTitle}</h2>{about.territoryParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</article>
      <article><span className="eyebrow">Realização</span><h2>{about.associationTitle}</h2>{about.associationParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</article>
    </section>
    <section className="location-section"><div><span className="eyebrow light">Onde acontece</span><h2>Perto de Belo Horizonte,<br />em meio à natureza.</h2><p>{settings.locationName}, {settings.locationRegion}.</p><a className="button blue" href={settings.mapUrl} target="_blank" rel="noreferrer">Abrir no mapa ↗</a></div><div className="map-art"><span>MINAS GERAIS</span><b>Macacos</b><i /></div></section>
