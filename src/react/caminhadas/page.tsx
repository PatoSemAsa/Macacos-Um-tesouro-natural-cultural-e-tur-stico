...
export default function CaminhadasPage() {
  ...
  const cmsTrails = agenda.filter((day) => day.href === item.href).map((day) => ({ date: day.date, name: day.title, time: day.items[0]?.time ?? "", level: day.label, text: day.items[0]?.text ?? day.place }));
  const trails = cmsTrails.length ? cmsTrails : fallbackTrails;
  return <main>
